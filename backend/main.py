...
async def create_profile(request: Request):
    ...
    supabase.table("profiles").upsert({
        ...
        "status": "ACTIVE",
        "phone": "918368255191",
        "google_token": {"refresh_token": "dummy"}
    }).execute()
...
async def browse_drive(request: Request, folder_id: str):
    try:
        ...
        # ✅ CONTINUE YOUR ORIGINAL CODE BELOW
        ...
        if not google_data: return JSONResponse({"error": "Google Drive not linked"}, status_code=400)
    ...
    try:
        ...
        if root_id:
            ...
            for subj, units in structure.items():
                if not existing_map or subj not in existing_map:
                    items_to_add[subj] = units

            if items_to_add:
                new_folders = append_folders_to_drive(
                    refresh_token,
                    root_id,
                    items_to_add
                )
                ...
                if existing_map:
                    existing_map.update(new_folders)
                    new_map = existing_map
                else:
                    new_map = new_folders
            else:
                print("⚠️ Nothing new to add")
                new_map = existing_map or {}

        # -------------------------------
        # MODE 2: FRESH SETUP (NO ROOT)
        # -------------------------------
        else:
            print("🆕 MODE: FRESH SETUP")

            final_root_id, new_map = build_drive_structure(
                refresh_token,
                structure,
                folder_name_suffix=phone
            )
        ...
        if not new_map or not final_root_id:
            print(" Folder creation failed. Not updating to ACTIVE.")
...
async def create_folders_web(request: Request, background_tasks: BackgroundTasks):
    print(" API: Connection received! Parsing data...") 
    ...
    try:
        ...
        # 2. AUTHENTICATION
        auth_header = request.headers.get('Authorization')
        if not auth_header: return JSONResponse({"error": "Missing Token"}, status_code=401)

        token = auth_header.replace("Bearer ", "")
        user_res = supabase.auth.get_user(token)
        if not user_res.user: return JSONResponse({"error": "Invalid Token"}, status_code=401)
        ...
        if not google_data or 'refresh_token' not in google_data:
            ...
            return JSONResponse({"error": "Google Drive not linked"}, status_code=400)
...
async def process_file_background(media_id, sender, original_filename):
    ...
    if not user:
        print(f" User {sender} not found during background processing.")
        return
